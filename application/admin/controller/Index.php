            ...
            if ($this->request->isPost()) {
                ...
                switch ($_POST['type']) {
                    ...
                    case 'polylineChart':   //折线图
                        $polylineData = $this->saleInfo->alias('i')
                            ->join('wugu_goods g', 'i.goods_id = g.id', 'LEFT')
                            ->field('sum(i.sale_total_money) as total_money,sum(g.cost*i.sale_count) as cost,sum(ABS(i.sale_total_money - g.cost*i.sale_count)) as profit,FROM_UNIXTIME(i.sale_time,\'%Y-%m-%d\') as sale_time')
                            ->where($where)
                            ->group('FROM_UNIXTIME(i.sale_time,\'%Y-%m-%d\')')
                            ->select();
                        foreach ($dataArr as $k => &$v) {
                            foreach ($polylineData as $m => $n) {
                                if ($v['sale_time'] == $n['sale_time']) {
                                    $v['total_money'] = $n['total_money'];
                                    $v['cost']        = $n['cost'];
                                    $v['profit']      = $n['profit'];
                                    ...
                                    $v['profit']      = empty($v['profit']) ? 0 : $v['profit'];
                                }
                            }
                        }
                        $xAxis = array_column($dataArr,'sale_time');
                        $total_money = array_column($dataArr,'total_money');
                        $cost = array_column($dataArr,'cost');
                        $profit = array_column($dataArr,'profit');
                        return ['status' => true, 'xAxis' => $xAxis, 'total_money' => $total_money,'cost'=>$cost,'profit'=>$profit];
                        break;
                }
